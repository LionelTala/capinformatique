...
import { ArrowLeftIcon, CalendarIcon, DocumentIcon, PlusIcon, UserGroupIcon, UserIcon, XMarkIcon } from "@heroicons/react/24/outline";
//#region resources/js/Pages/Admin/Devoirs/Edit.tsx
function Edit({ devoir, formations }) {
	var _devoir$formation_id, _devoir$vague_id, _devoir$certification, _devoir$student_id, _devoir$tranche_requi;
	...
	const { data, setData, processing, errors } = useForm({
		...
		formation_id: ((_devoir$formation_id = devoir.formation_id) === null || _devoir$formation_id === void 0 ? void 0 : _devoir$formation_id.toString()) || "",
		...
		student_id: ((_devoir$student_id = devoir.student_id) === null || _devoir$student_id === void 0 ? void 0 : _devoir$student_id.toString()) || "",
		tranche_requise_id: ((_devoir$tranche_requi = devoir.tranche_requise_id) === null || _devoir$tranche_requi === void 0 ? void 0 : _devoir$tranche_requi.toString()) || "",
	...
	useEffect(() => {
		if (data.formation_id && data.type_destinataire === "vague") {
			...
			fetch(`/admin/devoirs/vagues/${data.formation_id}`).then((res) => {
				...
				if (devoir.vague_id && list.some((v) => v.id === devoir.vague_id)) setData("vague_id", devoir.vague_id.toString());
			}).catch(() => {
				setVagues([]);
				setLoadingVagues(false);
			});
	...
	useEffect(() => {
		if (data.formation_id && data.type_destinataire === "certification") {
			...
			fetch(`/admin/devoirs/certifications/${data.formation_id}`).then((res) => {
				if (!res.ok) throw new Error(`HTTP ${res.status}`);
				return res.json();
			}).then((data) => {
				const list = Array.isArray(data) ? data : [];
				...
				if (devoir.certification_id && list.some((c) => c.id === devoir.certification_id)) setData("certification_id", devoir.certification_id.toString());
			}).catch(() => {
				setCertifications([]);
				setLoadingCertifications(false);
			});
	...
	useEffect(() => {
		if (data.formation_id) {
			...
			fetch(`/admin/devoirs/tranches/${data.formation_id}`).then((res) => {
				if (!res.ok) throw new Error(`HTTP ${res.status}`);
				return res.json();
			}).then((data) => {
				const list = Array.isArray(data) ? data : [];
				...
				if (devoir.tranche_requise_id && list.some((t) => t.id === devoir.tranche_requise_id)) setData("tranche_requise_id", devoir.tranche_requise_id.toString());
			}).catch(() => {
				setTranches([]);
				setLoadingTranches(false);
			});
	...
	useEffect(() => {
		if (data.certification_id && data.type_destinataire === "certification") {
			...
			fetch(`/admin/devoirs/students-by-certification/${data.certification_id}`).then((res) => {
				if (!res.ok) throw new Error(`HTTP ${res.status}`);
				return res.json();
			}).then((data) => {
				const list = Array.isArray(data) ? data : [];
				...
				if (devoir.student_id && list.some((s) => s.id === devoir.student_id)) setData("student_id", devoir.student_id.toString());
			}).catch(() => {
				setStudents([]);
				setLoadingStudents(false);
			});
				...
				children: /* @__PURE__ */ jsxs("form", {
					...
					children: [
						...
						data.type_destinataire === "vague" && /* @__PURE__ */ jsxs(Fragment, { children: [/* @__PURE__ */ jsxs("div", { children: [
							...
							/* @__PURE__ */ jsx("p", {
								className: "text-xs text-gray-400 mt-1",
								children: "Les étudiants devront avoir payé cette tranche pour accéder au devoir"
							}),
						...
						data.type_destinataire === "certification" && /* @__PURE__ */ jsxs(Fragment, { children: [
							...
							/* @__PURE__ */ jsxs("div", {
								...
								children: [/* @__PURE__ */ jsx("p", {
									...
									children: "Les étudiants en certification ont accès à tous les devoirs sans restriction de tranche."
								})]
							})
						] }),
						/* @__PURE__ */ jsxs("div", { children: [
						...
						/* @__PURE__ */ jsx("button", {
							...
							children: processing ? "Mise à jour en cours..." : "Mettre à jour le devoir"
						})
					]
				})
			})]
...
//# sourceMappingURL=Edit-eeqNUv9f.js.map