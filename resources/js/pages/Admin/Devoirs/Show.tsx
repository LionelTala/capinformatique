// resources/js/pages/Admin/Devoirs/Show.tsx
import {
    ArrowLeftIcon,
    DocumentIcon,
    UserIcon,
...
export default function Show({ devoir, soumissions, nonSoumis }: Props) {
    ...
    return (
        <>
            ...
            <AdminLayout title={`Détails du devoir`}>
                <div className="max-w-5xl">
                        ...
                        <div className="flex items-start justify-between">
                                ...
                                <div className="flex items-center gap-4 mt-2 text-sm">
                                    {devoir.date_limite && (
                                        <span className={`flex items-center gap-1 ${devoir.est_depasse ? 'text-red-500' : 'text-gray-500'}`}>
                                            ...
                                            Date limite : {devoir.date_limite}
                            ...
                            <div className="flex gap-2">
                                {!devoir.has_notification_sent && (
                                    <button
                                        onClick={() => {
                                            if (confirm(`Renvoyer les notifications pour le devoir "${devoir.titre}" ?`)) {
                                                router.post(`/admin/devoirs/${devoir.id}/resend-notifications`);
                                            }
                                        }}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 text-center">
                            <p className="text-2xl font-bold text-cab-blue">{devoir.soumissions_count}</p>
                            <p className="text-xs text-gray-500">Soumissions</p>
                        </div>
                        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 text-center">
                            <p className="text-2xl font-bold text-orange-500">{devoir.soumis_count}</p>
                            <p className="text-xs text-gray-500">En attente de correction</p>
                        </div>
                        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 text-center">
                            <p className="text-2xl font-bold text-green-500">{devoir.corrige_count}</p>
                            <p className="text-xs text-gray-500">Corrigés</p>
                        </div>
                        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 text-center">
                            <p className="text-2xl font-bold text-purple-500">{devoir.taux_soumission}%</p>
                ...
                {showCorrectionModal !== null && (
                    <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                        <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl">
                            <h3 className="text-lg font-bold text-gray-900 mb-4">Corriger le devoir</h3>
                            <p className="text-sm text-gray-600 mb-4">
                                Entrez la note et éventuellement un commentaire pour ce devoir.
                            </p>
